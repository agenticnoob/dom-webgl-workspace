...
- [`apps/hero-next`](../apps/hero-next/) - private Next.js App Router consumer with a managed `WebGLMesh` tetrahedron, background Ghost Cursor, two fixed directional lights, an effect-owned pointer light, and app-owned breathing/floating/pointer-parallax motion without continuous self-rotation; the latest `opacity: 0.92` alpha experiment is automated-verified, but user visual QA found a white/milky metallic result rather than physical transmission.

...
- [`apps/hero-next`](../apps/hero-next/) - private Next.js App Router consumer with a strict `#B8B8B8` / `#5F5F5F` semantic palette and a reversible hold-driven radial toggle. A real primary-pointer `WebGLMesh` hit drives one pure effect-owned state machine; six shared progress signals drive the aspect-correct Ghost Cursor circle. The tetrahedron effect's matching material/emissive writes are authored but skipped because the real scene-native mesh runtime does not yet expose `ctx.object.material`. The app keeps the public scroll runtime and future Lenis/GSAP/ScrollTrigger capability without transition pinning or scroll space, preserves the existing lights and ambient motion, removes shake under reduced motion, and retains opt-in antialiasing at maximum DPR `2`.

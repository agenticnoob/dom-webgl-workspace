...
- [`apps/hero-next`](../apps/hero-next/) - private Next.js App Router consumer with a strict `#B8B8B8` / `#5F5F5F` semantic palette and a reversible hold-driven radial toggle. A real primary-pointer `WebGLMesh` hit drives one pure effect-owned state machine; six shared progress signals drive the aspect-correct Ghost Cursor circle. The scene-native mesh runtime now injects the controlled material facade, so the tetrahedron effect's matching material/emissive writes reach the real runtime-owned `MeshStandardMaterial` without exposing raw Three handles. The background and Ghost Cursor use a per-fragment radial mask, while the tetrahedron currently changes the whole material at attempt start. A synchronized screen-space tetrahedron diffusion is the approved next design direction, but it still requires a separately designed general managed-mesh mask/program capability and is not implemented. The app keeps its existing standard descriptor, public scroll runtime, future Lenis/GSAP/ScrollTrigger capability, lights, ambient motion, reduced-motion rules, and opt-in antialiasing at maximum DPR `2`.

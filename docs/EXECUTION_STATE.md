...
## Last Completed Task
Lenis smooth-scroll lifecycle simplification and handoff cleanup.
...
behind one opt-in lifecycle object. Demo `ScrollZoomImage` now uses that smooth
scroll stack through `useDemoSmoothScrollStack(...)`, where the demo owns the
Lenis instance lifecycle and passes it to the adapter stack with
`manageLenis: false`; the hook currently uses `lerp: 0.055` and
`wheelMultiplier: 0.85` for a more responsive smooth-scroll feel. The gallery
moves through the local `demo.scrollGallery` effect rather than CSS transform

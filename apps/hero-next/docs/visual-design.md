...
4. **我正在连接什么**：抖音、小红书、哔哩哔哩、博客与 GitHub 的单行大字目录，悬停显示跟随鼠标的预览。
...
第四章以抖音、小红书、哔哩哔哩、博客、GitHub 五行大字作为正文，保持既有双色与 Arial / Helvetica 字体。
五行集中在一屏中部，悬停时整行反相高亮，预览跟随文字区域内的鼠标，并在视口边缘限制位置。
窗口沿当前行垂直居中，鼠标离开文字移入预览时停止跟随，便于直接点击。
文字与窗口属于同一个原生链接，点击或键盘 Enter 直接打开对应地址；没有点击固定、关闭按钮或二次确认。
触屏对已有链接直接跳转。抖音、小红书、哔哩哔哩暂未配置地址，保留可聚焦的不可用链接与
“链接待补充”预览提示，不生成虚构 URL。鼠标离开入口与其窗口、焦点离开、Escape、滚动或视口变化关闭预览。

正文采用 `140svh` 的滚动区与 `100svh` sticky 目录，五行各占 `12svh`；首尾 atlas 都绘制相同的未选中目录。
`src/signals/` 拥有目录交互、窗口边界计算与 atlas 绘制；入口文案和已确认链接继续由
`src/chapters/content.ts` 的 `signals` 内容持有。窄屏保留五个完整单行入口，隐藏辅助长句。
补充地址时在对应 section 添加 `link: { href, label }`，文字和窗口即可同时跳转。
预览正文暂用简介，后续可替换为确认后的二维码、图片或介绍。
DOM 只管理链接与窗口定位；pointer move 不进入 React state，无额外渲染器或动画时间线。
...
第三章桌面模式的两个端点都读取第一面正前方构图，移动端保留普通正文首尾；第四章首尾共用未选中的五行入口。Atlas 与语义 DOM 共用同一套响应式
...
- **第四章自动化：** 覆盖五行中英文入口、已确认链接、缺失地址的禁用行为、窗口与文字共用原生跳转、
  悬停跟随与移入窗口、Escape/滚动关闭、窄屏边界，以及首尾 atlas 的目录位置、灰度编码和文字基线。
- **第四章 Browser：** `2026-09-09` 生产构建在 ego-browser 中验证 `1440×900` 桌面悬停与窗口点击直达博客，
  `390×844` 英文窄屏与 reduced-motion 下五行均保持单行，无横向溢出，保留一个 canvas。
  文字点击已触发正确的 GitHub 目标 URL，但测试浏览器未能加载该外站，不计作外站可用性验证。
  应用 typecheck/build、import/docs 检查通过，React Doctor 为 100/100。未追加真机验证。
- **第三章自动化：** 覆盖边缘转向与重新触发、最短角度路径、首尾收束、reduced-motion、语义状态去重、键盘与 hover 锁定、DOM fallback、四个项目链接及双语纹理排版和源区域边界。
- **Automated：** hero-next 32 个 focused test files / 189 tests 覆盖第二章数据扩展、稳定文章 ID、
...
- **未声称：** 第一章未重验 `320×568`；本轮没有覆盖 iOS Safari、Android Chrome 真机、横屏或
  反向切回 initial theme；三个视频平台的目标地址待补充；Canvas 与 DOM 的字形抗锯齿不承诺
  像素级一致。

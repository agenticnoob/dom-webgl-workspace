...
describe("useExampleSmoothScrollStack", () => {
  const roots: Root[] = [];
  ...
  test("creates the official Lenis GSAP stack for the example under StrictMode", async () => {
    const { useExampleSmoothScrollStack } = await import("./useExampleSmoothScrollStack");
    ...
    await act(async () => {
      root.render(
        createElement(
          StrictMode,
          null,
          createElement(Harness, { useHook: useExampleSmoothScrollStack }),
        ),
      );
    });
    await flushFrame();
  ...
  test("disposes the adapter stack and app-owned Lenis instance on unmount", async () => {
    const { useExampleSmoothScrollStack } = await import("./useExampleSmoothScrollStack");
    const host = document.createElement("div");
    document.body.append(host);
    const root = createRoot(host);
    ...
    await act(async () => {
      root.render(createElement(Harness, { useHook: useExampleSmoothScrollStack }));
    });
    await flushFrame();

...
- 本项目是开源 DOM WebGL runtime。`apps/example` 是公共 API 的示例消费者和下游 dogfood 面，不能在 runtime/package 源码里写硬编码的 app 分支。
...
- `docs/README.md` — 文档索引：先区分 active docs 和 archive
- `docs/STATUS.md` — 当前实现真值和活跃方向摘要
- `docs/roadmap/managed-render-system.md` — 下一阶段长期 roadmap，后续迭代这里
- `docs/agent/package-onboarding.md` — 其他 agent 从零使用 packages 的单文档入口
- `docs/agent/package-usage.md` — 下游集成和 effect authoring 的详细 contract/reference
- `docs/agent/effect-object-boundary.md` — 当前 `ctx.object` effect authoring 边界
- `docs/examples/effect-authoring.md` — React-only 消费者教程
- `docs/00-goal.md` — 完整架构原则和非目标清单
- `README.md` — 项目入口、行为描述、示例代码、验证命令
- `docs/agent/effect-authoring-example-report.md` — 已知摩擦点
- `docs/archive/` — 已完成计划和历史记录，只作证据，不作当前 truth
...
- 修改 runtime 行为时优先保持声明驱动和包级通用能力；example/app 需要的能力必须先抽象成公共 API、内部通用管线或测试夹具。
- 排查硬编码时同时检查 `packages/dom-webgl-runtime/src` 的非测试实现、`apps/example` 的公共 API 使用边界、README 和 `docs/STATUS.md`。
